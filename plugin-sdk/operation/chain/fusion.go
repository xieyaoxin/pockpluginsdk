...
var fusionCancel context.CancelFunc

func (FusionStateMachineApi) HandleFinishEvent() {
	fusionCancel()
}
...
func Fusion(mergeConfig model.SingleMergeConfig, MainForceEvaluate bool, FusionAfterCcThreshold bool, AteForceEvaluate bool) (bool, error) {
	...
	// todo 增加宠物检查
	ProtectArticle1, err1 := operation.GetProtectArticleByType(mergeConfig.ProtectType1)
	if err1 != nil {
		return false, err1
	}
	ProtectArticle2, err2 := operation.GetProtectArticleByType(mergeConfig.ProtectType2)
	if err2 != nil {
		return false, err2
	}
...
func Evaluate(Pet *model.Pet, EvaluateRoute string) (bool, error) {
	EvaluateArticle := pet.PetServiceInstance.GetPetEvaluateArticle(Pet, EvaluateRoute)
...
func MergeGod(Config model.MergeGodConfig) (bool, error) {
	...
	FusionResult := pet.PetServiceInstance.GetBattlePet()
...
func mergeDragon(Config *model.MergeDragonConfig) (*model.Pet, error) {
	...
	for MainPet.Cc < Config.EatPet.MainPetConfig.PetCc {
		// 针对部分宠物进行特殊处理
		for utils.SlicesContainsString(operation.FusionSpecialEvaluatePetNames, MainPet.Name) {
			// 吃一个册子
			...
			TempPet, _ := pet.PetServiceInstance.GetPetDetail(MainPet.Id)
		...
		if fusion && err == nil {
			CurrentMainPet := pet.PetServiceInstance.GetBattlePet()
...
func mergeWithPetType(config *model.SingleMergeConfig) (*model.Pet, error) {
	...
	switch PetType {
	case "LD":
		WXL := operation.GetPetFromCacheByPetName(operation.WXL_NAMES)
		if WXL != nil {
			return WXL, nil
		}
		// 先读缓存
		if len(operation.LD_EGG_ARTICLE_LIST) == 0 {
			return nil, errors.New("找不到龙蛋")
		}
		// 使用龙蛋
		for _, LD := range operation.LD_EGG_ARTICLE_LIST {
			if LD.ArticleCount > 0 {
				article.ArticleServiceInstance.UserArticle(LD)
				break
			}
		}
		// 检查身上的宠物
		list, _ := pet.PetServiceInstance.GetCarriedPetList()
		if len(list) > 1 {
	...
	case "WX":
		...
		if fusion {
			BattlePet := pet.PetServiceInstance.GetBattlePet()
...
func prepareMerge(Pet *model.Pet, config *model.MergePetConfig, ForceEvaluate bool) error {
	// 设置主站
	pet.PetServiceInstance.SetBattlePet(Pet.Id)
	// 吃经验
	...
	for {
		...
		articleSample := ExperienceArticles
		if articleSample.ArticleCount == 0 {
			continue
		}
		article.ArticleServiceInstance.UserArticle(articleSample)
...
func getFusionPet(Config *model.MergePetConfig) (*model.Pet, error) {
	...
	Pet, _ := pet.PetServiceInstance.GetPetDetail(Config.PetId)
